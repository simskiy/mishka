...
module.exports = function serve() {
  ...
  watch([`${config.dir.blocks}**/*.{scss, sass}`], { events: ['change'], delay: 100 }, series(
    compileSass,
  ));

  // Стили Блоков: добавление
  watch([`${config.dir.blocks}**/*.{scss, sass}`], { events: ['add'], delay: 100 }, series(
    writeSassImportsFile,
    compileSass,
  ));

  // Стилевые глобальные файлы: все события
  watch([`${config.dir.src}scss/**/*.{scss, sass}`, `!${config.dir.src}scss/style.sass`], { events: ['all'], delay: 100 }, series(
    compileSass,
  ));
