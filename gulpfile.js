...
function getFavicons(cb) {
  let faviconFile = `${dir.src}favicon/in/*.{jpg,jpeg,png,gif}`;
...
function writeSassImportsFile(cb) {
  ...
  if (diff.length) {
    ...
    styleImports += msg;
    fs.writeFileSync(`${dir.src}scss/style.sass`, styleImports);
...
function serve() {
  ...
  watch([`${dir.blocks}**/*.{scss, sass}`], { events: ['change'], delay: 100 }, series(
    compileSass,
  ));

  // Стили Блоков: добавление
  watch([`${dir.blocks}**/*.{scss, sass}`], { events: ['add'], delay: 100 }, series(
    writeSassImportsFile,
    compileSass,
  ));

  // Стилевые глобальные файлы: все события
  watch([`${dir.src}scss/**/*.{scss, sass}`, `!${dir.src}scss/style.scss`], { events: ['all'], delay: 100 }, series(
    compileSass,
  ));
